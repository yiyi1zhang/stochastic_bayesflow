...
import itertools
import conditionalSNF.core.SNF as SNF
...
class SCNF(nn.Module):
    ...
    def __init__(self, input_dim, cond_dim, summary_dim, conv_out, conv_kernel, gru_out,
                 skip_steps, reccs_out, ar_window, n_blocks, sub_net_size, log_posterior):
        super().__init__()
        self.input_dim = input_dim
        self.n_blocks = n_blocks
        self.cSNF = self.build_cSNF(n_blocks, sub_net_size, log_posterior, metr_steps_per_block=3,
                                    dimension_condition=summary_dim, dimension=input_dim, noise_std=0.1,
                                    num_inn_layers=1, lang_steps=0, lang_steps_prop=1, step_size=5e-3,
                                    langevin_prop=False)
        self.condNet = LSTNet_Encoder(cond_dim, summary_dim, conv_out, conv_kernel, gru_out, skip_steps, reccs_out, ar_window)

    def parameters(self):
        params = [self.condNet.parameters()]
        for k in range(len(self.cSNF)):
            params.append(self.cSNF[k].parameters())
        return itertools.chain(*params)

    def named_parameters(self):
        params = [self.condNet.named_parameters()]
        for k in range(len(self.cSNF)):
            params.append(self.cSNF[k].named_parameters())
        return itertools.chain(*params)

    def build_cSNF(self, n_blocks, sub_net_size, log_posterior, metr_steps_per_block=3, dimension_condition=5,
                    dimension=5, noise_std=0.4, num_inn_layers=1,  lang_steps=0, lang_steps_prop=1, step_size=5e-3,
                    langevin_prop=False):
        layers = []
        lambd = 1.
        for k in range(n_blocks):
            # lambd = (k + 1) / (n_blocks)
            layers.append(SNF.deterministic_layer(num_inn_layers, sub_net_size, dimension_condition=dimension_condition,
                                                  dimension=dimension))
        if metr_steps_per_block > 0:
            if lang_steps > 0:
                layers.append(SNF.Langevin_layer(log_posterior, lambd, lang_steps, step_size))
            if langevin_prop:
                layers.append(SNF.MALA_layer(log_posterior, lambd, metr_steps_per_block, lang_steps_prop, step_size))
            else:
                layers.append(SNF.MCMC_layer(log_posterior, lambd, noise_std, metr_steps_per_block))

        return layers

    def forward(self, x, y):
        cond = self.condNet(y)
        logdet = torch.zeros(len(x), device=device)
        zs = x.clone()
        for k in range(len(self.cSNF)):
            out = self.cSNF[k].forward(zs, cond)
            zs = out[0]
            logdet += out[1]
        return (zs, logdet)
    ...
    def backward(self, z, y):
        cond = self.condNet(y)
        logdet = torch.zeros(len(z), device=device)
        xs = z.clone()
        for k in range(len(self.cSNF) - 1, -1, -1):
            out = self.cSNF[k].backward(xs, cond)
            xs = out[0]
            logdet += out[1]
        return (xs, logdet)

    def sample(self, y, n_samples, z=None, device='cpu'):
        if z is None:
            z_samples = torch.randn(n_samples, y.shape[0], self.input_dim).to(device)
        else:
            z_samples = z.to(device)
        cond = self.condNet(y)
        x_samples = z_samples.clone()
        for k in range(len(self.cSNF) - 1, -1, -1):
            x_samples = self.cSNF[k].sample(x_samples, torch.stack([cond] * n_samples))
        return x_samples
    ...
    def save(self, name):
        state_dicts = {'cond_net': self.condNet.state_dict()}
        for k in range(len(self.cSNF)):
            state_dicts[f'cSNF_{k}'] = self.cSNF[k].state_dict()
    ...
    def load(self, name):
        ...
        for k in range(len(self.cSNF)):
            self.cSNF[k].load_state_dict(state_dicts[f'cSNF_{k}'])
...
def load_data(file_theta, file_sig, split0=300, split1=300, split2=400, split3=500, noise_ratio=0.0):
    """
    ...
    sig_c1, factor = normalize(sig_c1)
    ...
    for i in range(sig_c1.shape[-1]):
        X_test_noisy[:, :, i] += np.random.normal(0, noise_ratio * 0.5, size=(X_test.shape[0], X_test.shape[1]))
    ...
    return X_train, theta_train, X_val, theta_val, X_test, X_test_noisy, theta_test, factor
...
def train_model(model, optimizer, data_generator, val_generator, iterations, lr_scheduler, ep):
    ...
    for it in range(iterations):
        ...
        ml_loss, norm_loss, log_loss = maximum_likelihood_loss(Z, log_det_J, weight=1)
        ml_loss.retain_grad()
        optimizer.zero_grad()
        ml_loss.backward()
        optimizer.step()
    ...
    for X_val, theta_val in iter(val_generator):
        ...
        ml_val, norm_val, log_val = maximum_likelihood_loss(Z_val, log_det_J_val, weight=1)

    if 1000 < ep < 5 * max_explr_epoch:
        lr_scheduler.step()
...
def plot_distributions_cross(theta_approx, theta_test, filename=None, show=False):
    import matplotlib
    matplotlib.use('Qt5Agg')
    import matplotlib.pyplot as plt
    import seaborn as sns
    ...
    for i in range(n_rows):
        for j in range(n_columns):
            if j > i:
                ...
                for k in range(theta_approx.shape[0]):
                    l = sns.kdeplot(theta_approx[k, :, i], ax=ax[i, j], color='#b3e6ff')
                l = sns.kdeplot(theta_approx_mean[:, i], ax=ax[i, j], color='#5c92e8')
                l = sns.kdeplot(theta_test[:, i], ax=ax[i, j], color='#e55e5e')
                ax[i, j].spines['right'].set_visible(False)
                ax[i, j].spines['top'].set_visible(False)
                ax[i,j].set_ylabel("")
                if i == 0:
                    ax[i, j].legend([l.get_lines()[-3], l.get_lines()[-2],l.get_lines()[-1]],
                                 [r'$p_{est.}(\widehat{\theta}_{d}^{(k, b)})$', r'$p_{est.}(\widehat{\theta}_{d}^{(k)})$', r'$p_{true}(\theta_{d}^{(k)})}]$'])
            else:
                sns.kdeplot(x=theta_approx_mean[:, i], y=theta_approx_mean[:, j], ax=ax[i, j], fill=True)
                ax[i, j].spines['right'].set_visible(False)
                ax[i, j].spines['top'].set_visible(False)
...
def uncertainty_decomposition(model, X_test, theta_test, n_passes, n_samples):
    theta_test_approx_all = []
    z_samples = torch.randn(n_samples, X_test.shape[0], theta_dim)
    for i in range(n_passes):
        model(theta_test, X_test)
        theta_test_approx_all.append(model.sample(X_test, n_samples, z=z_samples).detach().cpu().numpy())
    theta_test_approx_all = np.stack(theta_test_approx_all, axis=0)
    aleatoric_var = theta_test_approx_all.var(axis=1).mean(axis=0).mean(axis=0)
    epistemic_var = theta_test_approx_all.mean(axis=1).var(axis=0).mean(axis=0)
    return theta_test_approx_all, aleatoric_var, epistemic_var
...
if __name__ == '__main__':
    ...
    init_bias = 0.001
    kernel_gamma = 0.5
    max_explr_epoch = 4000
    start_lr = 5e-4
    noise_ratio = 0

    data_path = 'data'
    vehicle = 3
    X_train, theta_train, X_val, theta_val, X_test, _, theta_test, factor = load_data(
        data_path + os.sep + f'vehicle_{vehicle}_param_STD_2000.npy',
        data_path + os.sep + f'vehicle_{vehicle}_data_STD_2000.npy',
        split0=train_size, split1=1600, split2=1800, split3=2000)


    def neg_log_posterior(xs, ys):
        # mu = model.condNet.O_mu
        # var = 100 * torch.exp(model.condNet.O_logvar)
        # p1 = 0.5 * torch.sum(torch.log(var), dim=-1)
        # p2 = 0.5 * torch.sum((mu - ys) ** 2 / var, dim=-1)
        p3 = torch.zeros((ys.shape[0])).to(device)
        return p3

    log_posterior = lambda xs, ys: neg_log_posterior(xs, ys)
    model = SCNF(theta_dim, cond_dim, summary_dim, conv_out, conv_kernel, gru_out,
                 skip_steps, reccs_out, ar_window, n_blocks, sub_net_size, log_posterior)
    ...
    train_loss = []
    val_loss = []
    early_stopping = EarlyStopping(patience=5000, verbose=True, delta=0.0001,
                                   path=f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip36_train_{train_size}_seq5.pt')
    model.to(device)
    ...
    np.save(
        f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip36_train_{train_size}_seq5_train_loss.npy',
        np.array(train_loss))
    np.save(
        f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip36_train_{train_size}_seq5_val_loss.npy',
        np.array(val_loss))

    # model.load(f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip6_train_{train_size}_seq5.pt')
    # theta_approx = model.sample(X_test, n_samples).detach().numpy()
    # np.save(f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip6_train_{train_size}_seq5_theta_approx.npy', theta_approx)
    # plot_inference_scatter_CI(theta_approx, theta_test, n_samples, param_names, figsize=(10, 4),
    #                           filename=f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip6_train_{train_size}_seq5_inference.png')
    # plot_distribution_data_set(theta_approx, theta_test,
    # filename=f'results/Bayesian_stochastic_conditional_normalizing_flow_MCMC_vehicle_{vehicle}_STD_skip6_train_{train_size}_seq5_distribution.png')
