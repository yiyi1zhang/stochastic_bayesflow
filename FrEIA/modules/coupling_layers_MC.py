...
class MCDropout(nn.Dropout):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return nn.functional.dropout(input, p=self.p, training=self.active)
...
class GaussianDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        epsilon = Variable(torch.randn(x.size()) * self.alpha + 1).to(x.device)
        if self.active:
            return x * epsilon
        else:
            return x
...
class RNVPCouplingBlock(_BaseCouplingBlock):
    ...
    def __init__(self, dims_in, dims_c=[],
                 subnet_constructor: Callable = None,
                 subnet_s_constructor: Callable = None,
                 subnet_t_constructor: Callable = None,
                 clamp: float = 2.,
                 clamp_activation: Union[str, Callable] = "ATAN"):
        '''
        Additional args in docstring of base class.
        ...
        Args:
          subnet_s_constructor: function or class, with signature
            constructor(dims_in, dims_out).  The result should be a torch
            nn.Module, that takes dims_in input channels, and dims_out output
            channels. See tutorial for examples. Four of these subnetworks will be
            initialized in the block.
          subnet_t_constructor: function or class, with signature
            constructor(dims_in, dims_out).  The result should be a torch
            nn.Module, that takes dims_in input channels, and dims_out output
            channels. See tutorial for examples. Four of these subnetworks will be
            initialized in the block.
        ...
        if subnet_constructor is not None:
            ...
            self.subnet_s1 = subnet_s_constructor(self.split_len1 + self.condition_length, self.split_len2)
            self.subnet_t1 = subnet_t_constructor(self.split_len1 + self.condition_length, self.split_len2)
            self.subnet_s2 = subnet_s_constructor(self.split_len2 + self.condition_length, self.split_len1)
            self.subnet_t2 = subnet_t_constructor(self.split_len2 + self.condition_length, self.split_len1)
...
class AffineCouplingOneSided(_BaseCouplingBlock):
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        a = self.subnet(x1_c)
        s, t = torch.split(a, [self.split_len1, self.split_len2], -1)
